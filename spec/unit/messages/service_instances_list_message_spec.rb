  ...
  RSpec.describe ServiceInstancesListMessage do
    ...
    describe 'validations' do
      ...
      context 'fields' do
        ...
        it 'accepts the `fields` parameter with `[space]=relationship.organization`' do
          message = described_class.from_params({ 'fields' => { 'space': 'relationship.organization' } })
          expect(message).to be_valid
          expect(message.requested?(:fields)).to be_truthy
          expect(message.fields).to match({ 'space': %w(relationship.organization) })
        end
