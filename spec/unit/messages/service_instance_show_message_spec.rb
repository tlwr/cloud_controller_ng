...
module VCAP::CloudController
  RSpec.describe ServiceInstanceShowMessage do
    ...
    it 'does not accept fields keys that are not allowed' do
      message = described_class.from_params({ 'fields' => { 'space.foo': 'name' } })
      expect(message).not_to be_valid
      expect(message.errors[:fields]).to include("valid keys are: 'space.organization', 'service_plan.service_offering', 'service_plan.service_offering.service_broker'")
    end
